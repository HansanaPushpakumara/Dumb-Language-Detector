...
%================= Skin color detection ===========================      
    ...
    img=rgb2ycbcr(img);%RGB to luminance and chrominance
    for i=1:size(img,1)
        for j= 1:size(img,2)
            cb = img(i,j,2);
            cr = img(i,j,3);
 ...
 %===================================================================== 
       ...
       bw = imfill(bw,'holes'); %filling uncompleted parts/holes
        ...
        imshow(bw);
        %bw2 = imcomplement(bw);
        imwrite(bw,'D:\Coding\Matlab Project\project new\project\tempImage.png');

        fname=strcat('D:\Coding\Matlab Project\project new\project\', 'tempImage.png');
        im=imread(fname);
